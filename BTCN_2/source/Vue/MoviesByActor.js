...
import SearchActor from "./SearchActor.js";
...
export default {
  ...
  methods: {
    async loadAllMovies(page) {
      ...
      if (this.movies.length > 0) this.$emit("handleHaveResult");
    },
  },
  mounted() {
  ...
  template: `
    <div>
        <h2 v-if="movies.length > 0" :class="isDarkMode?'text-white':''" class="mt-2">Movies by Actor</h2>
        <div class="row">

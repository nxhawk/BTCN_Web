...
export default {
  ...
  methods: {
    async loadAllMovies(page) {
      ...
      let data = await fetch(
        `search/name/${this.search}?per_page=6&page=${page}`
      );
      this.page = page;
      this.movies = [];
  ...
  template: `
    <div>
        <h2 v-if="movies.length > 0" :class="isDarkMode?'text-white':''" class="mt-2">Movies by actor "{{search}}"</h2>
        <div class="row">
            <div class="col-4 mb-4" v-for="movie in movies">
                <div class="card ms-auto me-auto" style="width: 19rem; height:100%">
                    <img style="cursor:pointer;" :src="movie.image" class="card-img-top" :alt="movie.title" @click="$emit('handleMovieDetail', movie)">
